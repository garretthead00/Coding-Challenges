...
var canCross = function (stones) {
  let k = 0;
  const path = stones.reduce((acc, curr, index, arr) => {
    if (curr === acc) {
      const nextStone = arr
      .filter(
          (x) => 
        ...
        .pop();
      k = nextStone - curr > k ? nextStone - curr : k;

      return nextStone || acc;
    }

    return acc;
  }, 0);
  return path === stones.pop()
};

const stones = [0,1,3,5,6,8,12,17] //[0,1,2,3,4,8,9,11];
